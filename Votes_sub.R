...
sp_ver_2016 <- select(merge(sp_ver_2016, votacao_candidato_munzona_2016_SP_sp_ver, by = "NR_CANDIDATO", all.x=T), "NM_URNA_CANDIDATO", "NR_CANDIDATO", "SG_PARTIDO", "DS_COMPOSICAO_COLIGACAO", "zs", "SUBPREFEITURA", "sp_codigo", "VOTOS", "DS_SIT_TOT_TURNO")
sum(is.na(sp_ver_2016$SUBPREFEITURA))
sp_ver_2016_g <- group_by(sp_ver_2016, NM_URNA_CANDIDATO, NR_CANDIDATO, SG_PARTIDO, DS_COMPOSICAO_COLIGACAO, SUBPREFEITURA, sp_codigo, DS_SIT_TOT_TURNO)
sp_ver_2016_t <- summarise(sp_ver_2016_g, VOTOS = sum(VOTOS))
sum(is.na(sp_ver_2016_t$SUBPREFEITURA))

sp_ver_2016_el <- subset(sp_ver_2016_t, DS_SIT_TOT_TURNO=="ELEITO POR QP" | DS_SIT_TOT_TURNO=="ELEITO POR MÉDIA", select = c("NM_URNA_CANDIDATO", "SUBPREFEITURA", "sp_codigo", "VOTOS"))
sp_ver_2016_el <- spread(sp_ver_2016_el, NM_URNA_CANDIDATO,  VOTOS)
sum(sp_ver_2016_el[33,3:57], na.rm = T)/sum(sp_ver_2016_el[1:32,3:57], na.rm = T)
sp_ver_2016_el <- sp_ver_2016_el[1:32,]
sp_ver_2016_el[is.na(sp_ver_2016_el)] <- 0

write.csv(sp_ver_2016_el, "VOTOS_SUB_ELEITOS_2016.csv", fileEncoding = "UTF-8")

sp_ver_2016_su <- subset(sp_ver_2016_t, DS_SIT_TOT_TURNO=="SUPLENTE", select = c("NM_URNA_CANDIDATO", "SUBPREFEITURA", "sp_codigo", "VOTOS"))
sp_ver_2016_su <- spread(sp_ver_2016_su, NM_URNA_CANDIDATO,  VOTOS)
sum(sp_ver_2016_su[33,3:679], na.rm = T)/sum(sp_ver_2016_su[1:32,3:679], na.rm = T)
sp_ver_2016_su <- sp_ver_2016_su[1:32,]
sp_ver_2016_su[is.na(sp_ver_2016_su)] <- 0

write.csv(sp_ver_2016_su, "VOTOS_SUB_SUPL_2016.csv", fileEncoding = "UTF-8")

sp_ver_2012 <- select(merge(votacao_secao_2012_SP_sp_ver_nom, ZONA_SECAO, by = "zs", all.x=T), "zs", "SUBPREFEITURA", "sp_codigo", "X14", "X15")
colnames(sp_ver_2012)[4] <- "X12"
colnames(sp_ver_2012)[5] <- "VOTOS"
sp_ver_2012 <- select(merge(sp_ver_2012, votacao_candidato_munzona_2012_SP_sp_ver, by = "X12", all.x=T), "X15", "X12", "X24", "X28", "zs", "SUBPREFEITURA", "sp_codigo", "VOTOS", "X22")
sum(is.na(sp_ver_2012$SUBPREFEITURA))
sp_ver_2012_g <- group_by(sp_ver_2012, X15, X12, X24, X28, SUBPREFEITURA, sp_codigo, X22)
sp_ver_2012_t <- summarise(sp_ver_2012_g, VOTOS = sum(VOTOS))
sum(is.na(sp_ver_2012_t$SUBPREFEITURA))

sp_ver_2012_el <- subset(sp_ver_2012_t, X22=="ELEITO POR QP" | X22=="ELEITO POR MÉDIA", select = c("X15", "SUBPREFEITURA", "sp_codigo", "VOTOS"))
sp_ver_2012_el <- spread(sp_ver_2012_el, X15,  VOTOS)
sum(sp_ver_2012_el[33,3:57], na.rm = T)/sum(sp_ver_2012_el[1:32,3:57], na.rm = T)
sp_ver_2012_el <- sp_ver_2012_el[1:32,]
sp_ver_2012_el[is.na(sp_ver_2012_el)] <- 0

write.csv(sp_ver_2012_el, "VOTOS_SUB_ELEITOS_2012.csv", fileEncoding = "UTF-8")

sp_ver_2012_su <- subset(sp_ver_2012_t, X22=="SUPLENTE", select = c("X15", "SUBPREFEITURA", "sp_codigo", "VOTOS"))
sp_ver_2012_su <- spread(sp_ver_2012_su, X15,  VOTOS)
sum(sp_ver_2012_su[33,3:824], na.rm = T)/sum(sp_ver_2012_su[1:32,3:824], na.rm = T)
sp_ver_2012_su <- sp_ver_2012_su[1:32,]
sp_ver_2012_su[is.na(sp_ver_2012_su)] <- 0

write.csv(sp_ver_2012_su, "VOTOS_SUB_SUPL_2012.csv", fileEncoding = "UTF-8")
